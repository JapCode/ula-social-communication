---
pageType: home
pageId: home
title: Página Principal
hero:
  eyebrow: Núcleo Universitario Rafael Rangel
  title_line1: Comunicación
  title_line2: Social
  description: Licenciatura en Comunicación Social — Formando profesionales
    críticos para el futuro de la comunicación de emergente.
  cta_text: Cómo Inscribirse
  cta_link: /admission/process
  secondary_text: Explorar
  secondary_link: "#noticias"
quickAccess:
  title: Accesos Rápidos
  items:
    - title: Inscripciones
      subtitle: Nuevos ingresos
      subtitle_desktop: Gestión de nuevos ingresos y reingresos
      href: /admission/process
      aria_label: Inscripciones - Nuevos ingresos
    - title: Eventos
      subtitle: Próximas actividades
      subtitle_desktop: Foros, charlas, talleres y actividades académicas
      href: /student-life/events
      aria_label: Eventos - Próximas actividades
    - title: Pénsum
      subtitle: Malla curricular
      subtitle_desktop: Malla curricular y plan de estudios
      href: /academics/curriculum
      aria_label: Pénsum - Malla curricular
    - title: Contenido Programático
      subtitle: Programas de materias
      subtitle_desktop: Programa anual de cada materia y planificación académica
      href: /academics/course-content
      aria_label: Contenido Programático - Programas de materias
news:
  image_alt: Estudiantes de Comunicación Social en actividad académica
  link_href: /portfolio/articles
  link_text: Leer artículo
  secondary:
    - category: Comunicación
      title: Periodismo digital y el futuro de la información local
      date: 2026-05-24
      href: /portfolio/articles
    - category: Eventos
      title: "Foro: Inteligencia Artificial en la comunicación pública"
      date: 2026-05-20
      href: /student-life/events
    - category: Comunidad
      title: "Comunicación comunitaria: voces desde el territorio"
      date: 2026-05-13
      href: /student-life/community
  title: Nuevas narrativas transmedia en el entorno digital
  tag: Actualidad
  image: /images/uploads/184786.jpg
  description: Un recorrido por las últimas investigaciones de nuestra facultad
    sobre el impacto de la tecnología en el periodismo moderno.
  category: Investigación
video:
  eyebrow: 'Laboratorio de computación "Teresita Jiménez" '
  title: Reinaguración
  badge: Destacado
  video_id: VrEm2DAT1NA?si=8U0PQFXgJChWQN7o
  duration: 3:28
  description: Nuestro laboratorio de computación ha sido renovado
footer:
  email: comunicacion@nurr.ula.ve
  phone: "+58 422 515-3760 "
  schedule: Lun–Vie 8:00 AM – 4:00 PM
  social_instagram: https://www.instagram.com/comunicacionsocialulanurr/
  social_twitter: https://x.com/ulanurr_cs
  social_youtube: https://youtube.com/@comunicacionsocialulanurr
---
